...
from src.database.schema import NATURAL_KEYS, TABLES, causal_audits, strategy_runs
...
from src.strategies.ensemble import EnsembleDecision, evidence_weight_rows
...
class StrategyPipeline:
    ...
    def ingest(self, options: IngestOptions, emit: EventSink | None = None) -> StageOutcome:
        ...
        if provider is None:
            ...
            self._persist_coverage_request(query, manifest, "unavailable", force=options.force)
        ...
        if not gaps:
            manifest = self.bars.manifest(query)
            self._persist_coverage_request(query, manifest, "complete", force=options.force)
        ...
        empty_force = options.force and inserted == 0
        if empty_force:
        ...
        self._persist_coverage_request(query, manifest, request_status, force=options.force)
    ...
    def evaluate(self, options: EvaluationOptions, emit: EventSink | None = None) -> StageOutcome:
        ...
        cached = tuple(self._cached_run(options.scope, item, manifest.dataset_hash) for item in registered)
        if all(cached) and not options.force:
            self._emit(emit, "progress", "evaluate", 1.0, "reused cached evaluation")
            return StageOutcome(
                "reused",
                "reused cached evaluation",
                dataset_hash=manifest.dataset_hash,
                strategy_run_id=str(cached[0]),
                strategy_run_ids=tuple(str(item) for item in cached if item is not None),
            )
        ...
        try:
            ...
            batch = self._evaluate_engines(options.scope, registered, query, manifest)
        ...
        with _lock_for(lock_identity):
            for _attempt in range(5):
                ...
                row = self._strategy_run_row(
                    ...
                    run_timestamp,
                    status="running",
                    metrics={"reservation_generation": run_generation, "state": scope.mode.value},
                    ended_at=None,
                )
                try:
    ...
    def _evaluate_engines(
        ...
        for item in registered:
            signals = item.generator(item.spec, bars, StrategyContext())
    ...
    def _persist_evaluation_batch(
        ...
        with _lock_for("strategy_evaluation_persistence"), self.database.engine.begin() as connection:
            ...
            for row in evidence_weight_rows(
                ...
                run_id, run_timestamp = reservations[str(row["strategy_id"])]
                natural = {
