...
deep_research_promotions = Table(
    ...
    Column("challenger_hash", String),
    Column("incumbent_hash", String),
    Column("deployment_state", String, nullable=False),
    Column("shadow_cohort_hash", String),
    Column("rollback_target_hash", String),
    Column("forward_evidence_reset", Boolean, nullable=False),
    Column("evaluated_at", DateTime(timezone=True), nullable=False),
...
execution_observations = Table(
    ...
    Column("requested_quantity", Numeric(38, 12), nullable=False),
    Column("filled_quantity", Numeric(38, 12), nullable=False),
    Column("predicted_total_cost_bps", Numeric(38, 12), nullable=False),
    Column("realized_total_cost_bps", Numeric(38, 12), nullable=False),
    Column("predicted_latency_ms", Numeric(38, 12), nullable=False),
    Column("realized_latency_ms", Numeric(38, 12), nullable=False),
    Column("missed_fill", Boolean, nullable=False),
    Column("observed_at", DateTime(timezone=True), nullable=False),
    Column("observation", JSON, nullable=False),
    *common_columns(),
    CheckConstraint("requested_quantity > 0", name="ck_execution_observation_requested_positive"),
    CheckConstraint("filled_quantity >= 0", name="ck_execution_observation_filled_nonnegative"),
    UniqueConstraint("session_id", "intent_id", "broker_order_id", name="uq_execution_observation_order"),
)
...
contextual_outcomes = Table(
    "contextual_outcomes",
    metadata,
    Column("outcome_id", String, primary_key=True),
    Column("content_hash", String, nullable=False),
    Column("dataset_hash", String, nullable=False),
    Column("protocol_hash", String, nullable=False),
    Column("code_hash", String, nullable=False),
    Column("config_hash", String, nullable=False),
    Column("source_decision_hash", String, nullable=False),
    Column("provider", String, nullable=False),
    Column("feed", String, nullable=False),
    Column("venue", String, nullable=False),
    ...
    Column("gross_return", Float, nullable=False),
    Column("modeled_cost", Float, nullable=False),
    Column("net_return", Float, nullable=False),
    Column("regime_probabilities", JSON, nullable=False),
    Column("evidence", JSON, nullable=False),
    *common_columns(),
    CheckConstraint("modeled_cost >= 0", name="ck_contextual_outcome_cost_nonnegative"),
    UniqueConstraint(
        "dataset_hash",
        "protocol_hash",
        "provider",
        ...
        name="uq_contextual_outcome_identity",
    ),
)

asset_eligibility_evidence = Table(
    "asset_eligibility_evidence",
    metadata,
    Column("eligibility_id", String, primary_key=True),
    Column("content_hash", String, nullable=False),
    Column("policy_hash", String, nullable=False),
    Column("input_hash", String, nullable=False),
    Column("provider", String, nullable=False),
    ...
    Column("direction", String, nullable=False),
    Column("effective_at", DateTime(timezone=True), nullable=False),
    Column("data_through", DateTime(timezone=True), nullable=False),
    Column("state", String, nullable=False),
    Column("quality_score", Float, nullable=False),
    Column("source_event_watermark", String, nullable=False),
    Column("evidence", JSON, nullable=False),
    *common_columns(),
    CheckConstraint("quality_score >= 0 AND quality_score <= 1", name="ck_asset_eligibility_quality"),
    UniqueConstraint("input_hash", "policy_hash", name="uq_asset_eligibility_input_policy"),
)

regime_posteriors = Table(
    "regime_posteriors",
    metadata,
    Column("posterior_id", String, primary_key=True),
    Column("content_hash", String, nullable=False),
    ...
    Column("interval", String, nullable=False),
    Column("decision_timestamp", DateTime(timezone=True), nullable=False),
    Column("feature_through", DateTime(timezone=True), nullable=False),
    Column("training_through", DateTime(timezone=True)),
    Column("status", String, nullable=False),
    ...
    *common_columns(),
    UniqueConstraint(
        ...
        "interval",
        "decision_timestamp",
        name="uq_regime_posterior_context",
    ),
)

contextual_estimates = Table(
    "contextual_estimates",
    metadata,
    Column("estimate_id", String, primary_key=True),
    Column("content_hash", String, nullable=False),
    Column("parent_estimate_id", String),
    Column("dataset_hash", String, nullable=False),
    Column("protocol_hash", String, nullable=False),
    Column("provider", String, nullable=False),
    Column("feed", String, nullable=False),
    ...
    Column("effective_at", DateTime(timezone=True), nullable=False),
    Column("evidence_through", DateTime(timezone=True), nullable=False),
    Column("mean_net_edge", Float, nullable=False),
    Column("lower_net_edge", Float, nullable=False),
    Column("uncertainty", Float, nullable=False),
    Column("effective_observations", Float, nullable=False),
    Column("evidence", JSON, nullable=False),
    *common_columns(),
    CheckConstraint("uncertainty >= 0", name="ck_contextual_estimate_uncertainty"),
    CheckConstraint("effective_observations > 0", name="ck_contextual_estimate_effective"),
)

contextual_covariances = Table(
    "contextual_covariances",
    metadata,
    Column("covariance_id", String, primary_key=True),
    Column("content_hash", String, nullable=False),
    ...
    Column("status", String, nullable=False),
    Column("observations", Integer, nullable=False),
    Column("strategy_ids", JSON, nullable=False),
    Column("matrix", JSON, nullable=False),
    Column("evidence", JSON, nullable=False),
    *common_columns(),
    CheckConstraint("observations >= 0", name="ck_contextual_covariance_observations"),
)

contextual_weights = Table(
    "contextual_weights",
    metadata,
    Column("contextual_weight_id", String, primary_key=True),
    Column("content_hash", String, nullable=False),
    Column("allocation_id", String, nullable=False),
    Column("context_hash", String, nullable=False),
    Column("dataset_hash", String, nullable=False),
    Column("protocol_hash", String, nullable=False),
    Column("provider", String, nullable=False),
    ...
    Column("effective_at", DateTime(timezone=True), nullable=False),
    Column("weight", Float, nullable=False),
    Column("cash_weight", Float, nullable=False),
    Column("evidence", JSON, nullable=False),
    *common_columns(),
    CheckConstraint("weight >= 0 AND weight <= 1", name="ck_contextual_weight_range"),
    CheckConstraint("cash_weight >= 0 AND cash_weight <= 1", name="ck_contextual_cash_range"),
    UniqueConstraint("allocation_id", "strategy_id", name="uq_contextual_allocation_strategy"),
)

portfolio_research_decisions = Table(
    "portfolio_research_decisions",
    metadata,
    Column("portfolio_decision_id", String, primary_key=True),
    Column("content_hash", String, nullable=False),
    Column("selection_id", String, nullable=False),
    Column("decision_hash", String, nullable=False),
    Column("context_hash", String, nullable=False),
    Column("symbol", String, nullable=False),
    Column("direction", String, nullable=False),
    ...
    Column("weight", Float, nullable=False),
    Column("exclusion_reasons", JSON, nullable=False),
    Column("evidence", JSON, nullable=False),
    *common_columns(),
    CheckConstraint("weight >= 0 AND weight <= 1", name="ck_portfolio_research_weight"),
    UniqueConstraint("selection_id", "decision_hash", name="uq_portfolio_selection_decision"),
)

contextual_learning_trials = Table(
    "contextual_learning_trials",
    metadata,
    Column("contextual_trial_id", String, primary_key=True),
    Column("content_hash", String, nullable=False),
    Column("global_trial_id", String, nullable=False, unique=True),
    Column("dataset_hash", String, nullable=False),
    Column("protocol_hash", String, nullable=False),
    Column("candidate_hash", String, nullable=False),
    Column("ordinal", Integer, nullable=False),
...
contextual_drift_events = Table(
    ...
    metadata,
    Column("drift_event_id", String, primary_key=True),
    Column("content_hash", String, nullable=False),
    Column("context_hash", String, nullable=False),
    Column("effective_at", DateTime(timezone=True), nullable=False),
    Column("status", String, nullable=False),
...
NATURAL_KEYS: dict[str, tuple[str, ...]] = {
    ...
    "contextual_outcomes": ("outcome_id",),
    "asset_eligibility_evidence": ("eligibility_id",),
    "regime_posteriors": ("posterior_id",),
    "contextual_estimates": ("estimate_id",),
    "contextual_covariances": ("covariance_id",),
