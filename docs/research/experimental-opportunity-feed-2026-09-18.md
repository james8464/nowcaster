...
| Data boundary | Only the verified contiguous tail of finalized bars, paired with a fresh quote from the same provider, feed, and symbol |
...
- Research is current, directional, available, and passes the no-repaint check.
- Market monitoring is healthy and the available bars are finalized and contiguous.
