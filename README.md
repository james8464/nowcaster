...
The [historical account replay](docs/historical-account-replay-2026-09-09.md) feeds the retained Bitcoin and Ether rules one historical hour at a time, letting paper trades open and close against a running balance. It keeps future prices out of earlier decisions, records losses and missing hours, and compares ordinary costs with doubled costs. This separate research tool does not change the live study or add a native trading button. The linked report distinguishes completed findings from pending evaluation and explains why replaying previously inspected history is not fresh proof of profitability.
